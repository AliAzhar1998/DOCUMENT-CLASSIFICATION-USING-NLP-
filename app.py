...
paths = r'C:\Users\Dell\Data Science\Project - 2\Peoplesoft resumes'
#dir_list = os.listdir(r'C:\Users\Dell\Data Science\Project - 2\Peoplesoft resumes')

from image import set_png_as_page_bg
#set_png_as_page_bg('images2.jpg')

images_source="images1.jfif"
def set_bg_hack_url():
    st.markdown(
        f"""
         <style>
...
if len(uploaded_files)>0:
    with st.spinner('Loading.....'):
        ...
        if len(uploaded_files) == 1:
            st.header('Resume - '+filename )
            st.write(readingtextdoc(pfinal))
            st.header('Skills Set - '+filename)
        else:
            st.header('Resume')
            st.write(new_data)
            st.header('Skills Set')
        clean_data=pd.read_csv('Clean_Resume.csv')
...
if len(uploaded_files)>0:
    ...
    new_data['Clean_Resume'] = new_data['Clean_Resume'].apply(EDA.remove_html_tags)
    new_data['Clean_Resume'] = new_data['Clean_Resume'].apply(EDA.remove_url)
    new_data['Clean_Resume'] = new_data['Clean_Resume'].apply(EDA.remove_punc)
    new_data['Clean_Resume'] = new_data['Clean_Resume'].apply(EDA.remove_stopwords)
...
if len(uploaded_files) > 0:
    ...
    #print(i)
        for j in clean_data['Resume']:
            resume_data=[]
            resume_data.append(j)
            if (i == resume_data) and (j == resume_data):
            #st.write(resume_data)
                skills_list.append(clean_data['Skills_set'])
        else:
            pass
...
#if len(uploaded_files)>1:
 #   wc=WordCloud(width=1000,height=1000,min_font_size=10,background_color='black',colormap='Set1')
